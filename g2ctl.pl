...
# print STDERR "file=$tfile t_index=$t_index\n";
	 ...
	 $indexlast = $t_index;
      ...
      elsif ( $index1 == $index2 && $t_index > $index2) {
            $index2 = $t_index;
            $file2 = $tfile;
            $indexlast = $t_index;
            $filelast = $tfile;
...
#  make inventory of times 1, 2 and last
   system "$wgrib2 $wflag $tflag -lev -lev0 -ctl_ens -ftime -ctl_inv \"$file1\" >$ListA";
   if ($file1 ne $file2) {
      system "$wgrib2 $wflag $tflag -lev -lev0 -ctl_ens -ftime -ctl_inv \"$file2\" >>$ListA";
   }
   if ($file2 ne $filelast) {
      system "$wgrib2 $wflag $tflag -lev -lev0 -ctl_ens -ftime -ctl_inv \"$filelast\" >>$ListA";
   }
}
else {
